...
function lib.Init(title,corner)
    ...
    local function createTab(tabName)
        ...
        c(btn,{Parent=tabBar, Size=UDim2.new(0,80,0,30), BackgroundColor3=Color3.fromRGB(50,50,50), Text=tabName, TextColor3=Color3.new(1,1,1), TextScaled=true, AutoButtonColor=true})
    ...
    local function addToggle(section,text,default,callback,keybind,mode)
        ...
        f.InputBegan:Connect(function(input)
            if input.UserInputType==Enum.UserInputType.MouseButton1 then
                if mode=="Hold" then toggled=true; box.BackgroundColor3=Color3.fromRGB(0,255,0); if callback then callback(true) end
                else toggleState() end
            end
        end)
        f.InputEnded:Connect(function(input)
            if mode=="Hold" and input.UserInputType==Enum.UserInputType.MouseButton1 then
                toggled=false; box.BackgroundColor3=Color3.fromRGB(255,0,0); if callback then callback(false) end
            end
        end)
        ...
        if keybind then
            keybinds[keybind] = function(inputState)
                ...
                elseif mode=="Hold" then
                    if inputState=="Begin" then toggled=true; box.BackgroundColor3=Color3.fromRGB(0,255,0); if callback then callback(true) end
                    elseif inputState=="End" then toggled=false; box.BackgroundColor3=Color3.fromRGB(255,0,0); if callback then callback(false) end end
                end
            end
        end
