...
function lib.Init(title,corner)
    ...
    local function addSlider(section,text,min,max,default,decimals,callback)
        ...
        bar.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; updateValue(input.Position.X) end end)
    ...
    local function addDropdown(section,text,options,callback)
        ...
        for i,opt in pairs(options) do
            ...
            c(optBtn,{Parent=dropFrame,Size=UDim2.new(1,0,0,25),Text=opt,TextColor3=Color3.new(1,1,1),BackgroundColor3=Color3.fromRGB(60,60,60),TextScaled=true,AutoButtonColor=true})
